...
class DES_Encryption{
    ...
    int num_leftShift[16] = { 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1 }; // so bit can dich chuyen cho moi lan lap
    ...
    // cac hang so lien quan den van ban 
    const int IP_t[64] = { 	58,50,42,34,26,18,10,2,          // bang hoan vi 
    ...
    const int E_t[48] = { 	32,1,2,3,4,5,            // bang mo rong 
                            4,5,6,7,8,9,
                            8,9,10,11,12,13,
                            12,13,14,15,16,17,
                            16,17,18,19,20,21,
    ...
    void expand_R(string r, string r32) // khai trien theo bang 
	{
        r = "";
        for (int j = 0; j < 48; j++)
        {
...
public:
    void encrypt(const string& plain_txt, const string& key){
        ...
        L_key[0] = shift_bit(key_firstHalf, num_leftShift[0]);  // dich chuyen cac bit sang trai 
        ...
        for (int i = 0; i < 16; i++){
            keys_56[i] = L_key[i] + R_key[i]; // tao khoa 56 bit
        }
        for (int i = 0; i < 16; i++){
            key_48[i] = "";
            for (int j = 0; j < 48; j++)
                key_48[i] += keys_56[i][pc_2[j] - 1]; // tao khoa 48 bit
        }
        // lam viec tren van ban don gian 

        string plain_txt_64 = Hex_to_Bin(plain_txt); // chuy?n d?i khóa t? hex 16 bit sang 64 bit

        string IP = ""; // uy quyen cho khoa 
        ...
        for (int j = 0; j <48; j += 6) // chia moi gia tri thanh 8 chuoi moi chuoi co 6 ky tu
            for (int k = j; k < j + 6; k++)
                s[0][j / 6] += R_xor_K[0][k];
...
int main()
{
    ...
    cout << "Nhap TEXT 16 ky tu viet bang Hex: ";
    ...
    cout << "Nhap KEY 16 ky tu viet bang Hex: ";
