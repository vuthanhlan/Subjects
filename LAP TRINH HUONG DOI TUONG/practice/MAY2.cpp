...
class NSX
{
	...
	friend void chen(MAYTINH a[100], int &n);
};
...
//chen truoc vi co tenNSX la IC
void chen(MAYTINH a[100], int &n)
{
	int d=0;
	for(int i=0; i<n; i++){
		if(strcmp(a[i].x.TenNSX,"IC")==0){
			d++;
		}
	}
	if(d==0) cout<<"Khong co may tinh hang IC";
	else{
		for(int i=0; i<n; i++){
			if(strcmp(a[i].x.TenNSX,"IC")==0){
				for(int j=n-1;j>=i; j--){
